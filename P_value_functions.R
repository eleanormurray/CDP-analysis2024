...
baseline_orig = values$baseline[values$model=="Original"]
...
pfunc$type <-ifelse(pfunc$model == "Original", "expert","algorithm")

f71<-ggplot(pfunc, aes(x = RD, y = p, color = model, linetype = type))+
  ...
  scale_color_viridis(discrete = TRUE, option = "A", name = "Covariate set") +
  scale_linetype_manual(values = c("solid", "dotdash"), name = "Covariate source")+
...
tv_se <-values$tv_se
tv_orig = values$tv[values$model=="Original"]
...
pfunc2$type <-ifelse(pfunc2$model == "Original", "expert","algorithm")

f72<-ggplot(pfunc2, aes(x = RD, y = p, color = model, linetype = type))+
  geom_line(linewidth= 1.25)+
  scale_x_continuous(limits=c(-4,11), breaks = c(-2, 0, tv_orig, 5, 10), labels = c(-2, 0, tv_orig, 5, 10))+
  scale_y_continuous(expand=c(0,0), limits = c(0,1.1), breaks = c(seq(0, 1, by=0.2), 0.05), labels = c(seq(0,1, by =0.2), 0.05))+
  geom_vline(aes(xintercept = 0), linewidth = 1, linetype = "dotted", color = grey(0.75, 1))+
  geom_hline(aes(yintercept = 0.05), linewidth = 1, linetype = "dotted", color = grey(0.75, 1))+
  scale_color_viridis(discrete = TRUE, option = "A", name = "Covariate set") +
